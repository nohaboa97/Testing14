...
public abstract class MinecraftServer extends RecursiveEventLoop<TickDelayedTask> implements ISnooperInfo, ICommandSource, AutoCloseable, Runnable {
   ...
   protected void func_213194_a(SaveHandler p_213194_1_, WorldInfo info, WorldSettings p_213194_3_, IChunkStatusListener p_213194_4_) {
      if (this.isDemo()) {
   ...
   private void applyDebugWorldInfo(WorldInfo worldInfoIn) {
      ...
      worldInfoIn.getGameRulesInstance().setOrCreateGameRule("doDaylightCycle", "false", this);
   }
      ...
      for(ServerWorld serverworld : this.getWorlds()) {
         ...
         if (serverworld.dimension.getType() == DimensionType.OVERWORLD || this.getAllowNether()) {
            ...
            if (this.tickCounter % 20 == 0) {
               ...
               this.playerList.sendPacketToAllPlayersInDimension(new SUpdateTimePacket(serverworld.getGameTime(), serverworld.getDayTime(), serverworld.getGameRules().getBoolean("doDaylightCycle")), serverworld.dimension.getType());
               this.profiler.endSection();
            }
         ...
         (this.timeOfLastDimensionTick.computeIfAbsent(serverworld.dimension.getType(), (p_212377_0_) -> {
            return new long[100];
