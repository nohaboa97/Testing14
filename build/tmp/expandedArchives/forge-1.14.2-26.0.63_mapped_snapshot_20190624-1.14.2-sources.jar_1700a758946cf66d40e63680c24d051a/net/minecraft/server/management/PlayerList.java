...
public abstract class PlayerList {
   ...
   public void initializeConnectionToPlayer(NetworkManager netManager, ServerPlayerEntity playerIn) {
      ...
      serverplaynethandler.sendPacket(new SJoinGamePacket(playerIn.getEntityId(), playerIn.interactionManager.getGameType(), worldinfo.isHardcore(), serverworld.dimension.getType(), this.getMaxPlayers(), worldinfo.getGenerator(), this.viewDistance, serverworld.getGameRules().getBoolean("reducedDebugInfo")));
   ...
   public void sendWorldInfo(ServerPlayerEntity playerIn, ServerWorld worldIn) {
      ...
      playerIn.connection.sendPacket(new SUpdateTimePacket(worldIn.getGameTime(), worldIn.getDayTime(), worldIn.getGameRules().getBoolean("doDaylightCycle")));
   ...
   public void setViewDistance(int viewDistanceIn, int p_217884_2_) {
      this.viewDistance = viewDistanceIn;
      this.sendPacketToAllPlayers(new SUpdateViewDistancePacket(viewDistanceIn));

      for(ServerWorld serverworld : this.server.getWorlds()) {
         if (serverworld != null) {
            serverworld.getChunkProvider().setViewDistance(viewDistanceIn, p_217884_2_);
         }
      }
