...
public class DebugRenderer {
   ...
   private boolean field_217743_z;
   private boolean field_217739_A;
   ...
   public boolean shouldRender() {
      return this.chunkBorderEnabled || this.pathfindingEnabled || this.waterEnabled || this.heightMapEnabled || this.collisionBoxEnabled || this.neighborsUpdateEnabled || this.lightEnabled || this.worldGenAttemptsEnabled || this.solidFaceEnabled || this.field_217743_z || this.field_217739_A;
   }
   ...
   public void renderDebug(long partialTicks) {
      if (this.pathfindingEnabled) {
         this.pathfinding.render(partialTicks);
      }
      ...
      if (this.waterEnabled) {
         this.water.render(partialTicks);
      }

      if (this.heightMapEnabled) {
         this.heightMap.render(partialTicks);
      }

      if (this.collisionBoxEnabled) {
         this.collisionBox.render(partialTicks);
      }

      if (this.neighborsUpdateEnabled) {
         this.neighborsUpdate.render(partialTicks);
      }

      if (this.caveEnabled) {
         this.cave.render(partialTicks);
      }

      if (this.structureEnabled) {
         this.structure.render(partialTicks);
      }

      if (this.lightEnabled) {
         this.light.render(partialTicks);
      }

      if (this.worldGenAttemptsEnabled) {
         this.worldGenAttempts.render(partialTicks);
      }

      if (this.solidFaceEnabled) {
         this.solidFace.render(partialTicks);
      }

      if (this.field_217739_A) {
         this.field_217742_n.render(partialTicks);
      }
