...
public class EndGatewayTileEntity extends EndPortalTileEntity implements ITickableTileEntity {
         ...
         for(int j = -radius; j <= radius; ++j) {
            if (i != 0 || j != 0 || allowBedrock) {
               for(int k = 255; k > (blockpos == null ? 0 : blockpos.getY()); --k) {
                  ...
                  if (Block.isOpaque(blockstate.getCollisionShape(worldIn, blockpos1)) && (allowBedrock || blockstate.getBlock() != Blocks.BEDROCK)) {
                     blockpos = blockpos1;
                     break;
                  }
               }
   ...
   private static BlockPos findSpawnpointInChunk(Chunk chunkIn) {
      ...
      for(BlockPos blockpos3 : BlockPos.getAllInBoxMutable(blockpos, blockpos1)) {
         ...
         if (blockstate.getBlock() == Blocks.END_STONE && !Block.isOpaque(chunkIn.getBlockState(blockpos4).getCollisionShape(chunkIn, blockpos4)) && !Block.isOpaque(chunkIn.getBlockState(blockpos5).getCollisionShape(chunkIn, blockpos5))) {
