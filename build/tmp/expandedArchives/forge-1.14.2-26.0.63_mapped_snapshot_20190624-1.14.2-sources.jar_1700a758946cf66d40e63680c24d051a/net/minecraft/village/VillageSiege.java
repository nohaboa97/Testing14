...
public class VillageSiege {
      ...
      while(true) {
         ...
         if (!playerentity.isSpectator()) {
            ...
            if (this.world.func_217483_b_(blockpos)) {
               ...
               if (vec3d != null) {
                  if (net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.event.village.VillageSiegeEvent(this, world, playerentity, vec3d))) return false;
                  break;
               }
            }
         }
   ...
   private Vec3d findRandomSpawnPos(BlockPos pos) {
      for(int i = 0; i < 10; ++i) {
         ...
         if (this.world.func_217483_b_(blockpos) && WorldEntitySpawner.canCreatureTypeSpawnAtLocation(EntitySpawnPlacementRegistry.PlacementType.ON_GROUND, this.world, blockpos, EntityType.ZOMBIE)) { //Forge: MC-154328 passing in null always returns false, so pass in ZOMBIE
