...
public class BigTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
   ...
   public BigTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> p_i49918_1_, boolean p_i49918_2_) {
         ...
         for(int k = -i; k <= i; ++k) {
            if (Math.pow((double)Math.abs(j) + 0.5D, 2.0D) + Math.pow((double)Math.abs(k) + 0.5D, 2.0D) <= (double)(p_208529_3_ * p_208529_3_)) {
               ...
               if (isAirOrLeaves(p_208529_1_, blockpos)) {
                  this.setLogState(p_208529_5_, p_208529_1_, blockpos, OAK_LEAVES, p_208529_4_);
               }
            }
         }
      }
   ...
   private int makeLimb(Set<BlockPos> p_208523_1_, IWorldGenerationReader p_208523_2_, BlockPos p_208523_3_, BlockPos p_208523_4_, boolean p_208523_5_, MutableBoundingBox p_208523_6_) {
      if (!p_208523_5_ && Objects.equals(p_208523_3_, p_208523_4_)) {
         ...
         for(int j = 0; j <= i; ++j) {
            ...
            if (p_208523_5_) {
               this.setLogState(p_208523_1_, p_208523_2_, blockpos1, OAK_LOG.with(LogBlock.AXIS, this.getLoxAxis(p_208523_3_, blockpos1)), p_208523_6_);
   ...
   private void makeFoliage(IWorldGenerationReader p_208525_1_, int p_208525_2_, BlockPos p_208525_3_, List<BigTreeFeature.FoliageCoordinates> p_208525_4_, MutableBoundingBox p_208525_5_, Set<BlockPos> p_208525_6_) {
