...
public class PhantomSpawner {
   ...
   public int spawnMobs(ServerWorld worldIn, boolean spawnHostileMobs, boolean spawnPeacefulMobs) {
      if (!spawnHostileMobs) {
         return 0;
      } else {
            ...
            if (worldIn.getSkylightSubtracted() < 5 && worldIn.dimension.hasSkyLight()) {
               ...
               for(PlayerEntity playerentity : worldIn.getPlayers()) {
                  if (!playerentity.isSpectator()) {
                     ...
                     if (!worldIn.dimension.hasSkyLight() || blockpos.getY() >= worldIn.getSeaLevel() && worldIn.func_217337_f(blockpos)) {
