...
public abstract class Entity extends net.minecraftforge.common.capabilities.CapabilityProvider<Entity> implements INameable, ICommandSource, net.minecraftforge.common.extensions.IForgeEntity {
   ...
   private Vec3d getAllowedMovement(Vec3d vec) {
      ...
      AxisAlignedBB axisalignedbb1 = axisalignedbb.expand(vec).grow(1.0E-7D);
      Stream<VoxelShape> stream1 = this.world.getEntitiesWithinAABBExcludingEntity(this, axisalignedbb1).stream().filter((p_213285_1_) -> {
         return !this.isRidingSameEntity(p_213285_1_);
      }).flatMap((p_213280_1_) -> {
         return Stream.of(p_213280_1_.getCollisionBoundingBox(), this.getCollisionBox(p_213280_1_));
      }).filter(Objects::nonNull).filter(axisalignedbb1::intersects).map(VoxelShapes::create);
      ReuseableStream<VoxelShape> reuseablestream = new ReuseableStream<>(Stream.concat(stream1, stream));
      Vec3d vec3d = vec.lengthSquared() == 0.0D ? vec : getAllowedMovement(vec, axisalignedbb, this.world, iselectioncontext, reuseablestream);
      ...
      if (this.stepHeight > 0.0F && flag3 && (flag || flag2)) {
         Vec3d vec3d1 = getAllowedMovement(new Vec3d(vec.x, (double)this.stepHeight, vec.z), axisalignedbb, this.world, iselectioncontext, reuseablestream);
         Vec3d vec3d2 = getAllowedMovement(new Vec3d(0.0D, (double)this.stepHeight, 0.0D), axisalignedbb.expand(vec.x, 0.0D, vec.z), this.world, iselectioncontext, reuseablestream);
         if (vec3d2.y < (double)this.stepHeight) {
            Vec3d vec3d3 = getAllowedMovement(new Vec3d(vec.x, 0.0D, vec.z), axisalignedbb.offset(vec3d2), this.world, iselectioncontext, reuseablestream).add(vec3d2);
         ...
         if (func_213296_b(vec3d1) > func_213296_b(vec3d)) {
            return vec3d1.add(getAllowedMovement(new Vec3d(0.0D, -vec3d1.y + vec.y, 0.0D), axisalignedbb.offset(vec3d1), this.world, iselectioncontext, reuseablestream));
         }
      }
      ...
      if (this.world instanceof ServerWorld) {
         ...
         if (this.inPortal) {
            if (this.world.getServer().getAllowNether() && !this.isPassenger() && this.portalCounter++ >= i) {
               ...
               this.timeUntilPortal = this.getPortalCooldown();
               this.changeDimension(this.world.dimension.getType() == DimensionType.NETHER ? DimensionType.OVERWORLD : DimensionType.NETHER);
               this.world.getProfiler().endSection();
            }
