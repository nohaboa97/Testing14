...
public class RandomPositionGenerator {
   ...
   private static Vec3d generateRandomPos(CreatureEntity creature, int p_191379_1_, int p_191379_2_, @Nullable Vec3d p_191379_3_, boolean p_191379_4_, double p_191379_5_, ToDoubleFunction<BlockPos> p_191379_7_) {
         ...
         if (blockpos != null) {
            ...
            if (creature.detachHome() && p_191379_1_ > 1) {
               ...
               if (creature.posX > (double)blockpos1.getX()) {
                  i1 -= random.nextInt(p_191379_1_ / 2);
               } else {
                  i1 += random.nextInt(p_191379_1_ / 2);
               }

               if (creature.posZ > (double)blockpos1.getZ()) {
                  k1 -= random.nextInt(p_191379_1_ / 2);
               } else {
                  k1 += random.nextInt(p_191379_1_ / 2);
               }
            }

            BlockPos blockpos2 = new BlockPos((double)i1 + creature.posX, (double)j1 + creature.posY, (double)k1 + creature.posZ);
            if ((!flag || creature.isWithinHomeDistanceFromPosition(blockpos2)) && pathnavigator.canEntityStandOnPos(blockpos2)) {
               if (!p_191379_4_) {
               ...
               double d1 = p_191379_7_.applyAsDouble(blockpos2);
               if (d1 > d0) {
                  d0 = d1;
                  i = i1;
                  j = j1;
      ...
      if (flag1) {
         return new Vec3d((double)i + creature.posX, (double)j + creature.posY, (double)k + creature.posZ);
      } else {
         return null;
      }
   }
