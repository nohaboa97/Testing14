...
public class FleeTask extends Task<CreatureEntity> {
   ...
   protected boolean shouldExecute(ServerWorld worldIn, CreatureEntity owner) {
      ...
      return owner.getDistanceSq(entity) < 16.0D;
   }
   ...
   public static void func_220540_a(CreatureEntity p_220540_0_, Entity p_220540_1_, float p_220540_2_) {
      for(int i = 0; i < 10; ++i) {
         Vec3d vec3d = new Vec3d(p_220540_0_.posX, p_220540_0_.posY, p_220540_0_.posZ);
         Vec3d vec3d1 = new Vec3d(p_220540_1_.posX, p_220540_1_.posY, p_220540_1_.posZ);
         Vec3d vec3d2 = vec3d.subtract(vec3d1).normalize();
         Vec3d vec3d3 = RandomPositionGenerator.findRandomTargetTowardsScaled(p_220540_0_, 16, 7, vec3d2, (double)((float)Math.PI / 10F));
         if (vec3d3 != null) {
            p_220540_0_.getBrain().setMemory(MemoryModuleType.WALK_TARGET, new WalkTarget(vec3d3, p_220540_2_, 0));
            return;
         }
      }
